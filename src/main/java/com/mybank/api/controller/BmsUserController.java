...
public class BmsUserController {
	...
	public ResponseEntity<String> signup(@RequestBody UserProfile userProfile) {
		HttpHeaders headers = new HttpHeaders();
		try {
			ResponseEntity<String> response = userProfileService.registerUser(userProfile);

			// Post Registration Authorization token can be created. But for now
			// we can ask user to login.
			return response;
		} catch (Exception exception) {
			return new ResponseEntity<String>("Registration Failed " + exception.getMessage(), headers,
	 ...
	 * 
	 * @param token,
	 *            To make it stateless each request will carry token and
	 *            necessary information can beretrived from DB using token.
	 * @param userProfile
	 * @return
	 */
	...
	public ResponseEntity<AuthResponse> updateProfile(@RequestHeader("authorization") String token,
		...
		HttpHeaders headers = new HttpHeaders();
		try {
			// We can have additional level of authentication, like again check
			// authorization header is present in session collection.

			boolean status = userProfileService.updateUserProfile(userProfile);
			return new ResponseEntity<AuthResponse>(new AuthResponse("Update Profile Successful"), headers,
					HttpStatus.OK);
		} catch (Exception exception) {
